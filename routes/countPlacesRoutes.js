...
const atualizarTabelasInventario = async (session, countPlaces) => {
    ...
    let idItemsIvent
    const lZerar = countPlaces.zerar ? countPlaces.zerar : false
    ...
    if (!itemsInventory) {
        ...
        let itemsInvent = new ItemsInventory({
            ...
            quantidade: lZerar ? 0 : countPlaces.quantidade,            
            usuarioCriacao: countPlaces.usuarioCriacao,
        ...
        await ItemsInventory.findByIdAndUpdate(
                                idItemsIvent,
                                { $inc: { quantidade: lZerar ? 0 : countPlaces.quantidade } },
                                { $set: 
                                    {usuarioAlteracao: countPlaces.usuarioCriacao,
